...
main = do
  runTest do
    ...
    suite "Linear interpolation" do
      test "[]"
        $ Assert.equal (Tuple 0 List.mempty)
        $ linearInterpolate (List.fromFoldable []) 1.0
      test "[1 2, 3 4]"
        $ Assert.equal (Tuple 2 (List.fromFoldable [ Tuple 1.0 2.0, Tuple 2.0 3.0, Tuple 3.0 4.0 ]))
        $ linearInterpolate (List.fromFoldable [ Tuple 1.0 2.0, Tuple 3.0 4.0 ]) 1.0
      test "[1 10000, 3 2, 9 100]"
        $ Assert.equal (Tuple 2 (List.fromFoldable [ Tuple 2.0 (-10.0), Tuple 3.0 (-7.5), Tuple 4.0 (-5.0), Tuple 5.0 (-2.5), Tuple 6.0 0.0, Tuple 7.0 2.5, Tuple 8.0 5.0, Tuple 9.0 7.5, Tuple 10.0 10.0 ]))
    ...
    suite "Newton interpolation" do
      ...
      test "[1 2, 3 4]"
        $ Assert.equal (Tuple 2 List.mempty)
